...
//------------------ Funcao de insercao por cor
void insercaoPorCor(Pokemon *pok[], int n, int cor, int h){
    for (int i = (h + cor); i < n; i+=h) {
        Pokemon *tmp = pok[i];
        int j = i - h;

        // se o nome do pokemon de j for maior que o de tmp
        bool resp = (strcmp(getName(pok[j]), getName(tmp)) > 0);

        //testa se peso[j] e' diferente ao peso[tmp], se for igual ordena pelo nome
        while (j >= 0 && (getWeight(pok[j]) > getWeight(tmp) || (getWeight(pok[j]) == getWeight(tmp) && resp)) ) 
        {  
            pok[j + h] = pok[j];
            movimentacoes++; //conta as movimentacoes
            comparacoes++; //conta as comparacoes;

            //se a primeira for falsa conta mais duas comparacoes
            if (getWeight(pok[j]) == getWeight(tmp)) {
                comparacoes+= 2; // para j == tmp e resp = true
            }
            j -= h;
        }
        pok[j + h] = tmp;
...
//---------------- Metodo para ordenar pokemons pelo weight - Shellsort
void shellsort(Pokemon **pok, int n) {
    int movimentacoes = 0;
    int h = 1;
    ...
    do {
        ...
        for(int cor = 0; cor < h; cor++){
            //chamar metodo de insercao por cor
            insercaoPorCor(pok, n, cor, h);
        }
    } while (h != 1);
}

//------ Arquivo com numero de movimentacoes e tempo de execucao
void gravarArquivo(double inicio, double fim){
    FILE *arquivo = fopen("matricula_shellsort.txt", "wt");;
...
int main(){
    ...
    // Marcando tempo de execucao para shellsort //
    inicio += clock();  
    shellsort(pokemon, n); 
    fim += clock();
