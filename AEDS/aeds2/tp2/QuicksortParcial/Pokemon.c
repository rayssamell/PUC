...
//-------------------- Ordenar pela Generation - Quicksort
int quicksortRec(Pokemon **pokemon, int esq, int dir) {
    int movimentacoes = 0;

    int i = esq, j = dir;
    char *pivoNome = getName(pokemon[(dir+esq)/2]);
    int pivo = getGeneration(pokemon[(dir+esq)/2]); 
 
    while (i <= j) {
        // 1. Primeiro, faz a comparação pela geração
        while (getGeneration(pokemon[i]) < pivo || (getGeneration(pokemon[i]) == pivo && strcmp(getName(pokemon[i]), pivoNome) < 0)) i++;
        while (getGeneration(pokemon[j]) > pivo || (getGeneration(pokemon[j]) == pivo && strcmp(getName(pokemon[j]), pivoNome) > 0))  j--;

        
        if (i <= j) {
            swap(pokemon, i, j);
            movimentacoes += 3;  // Considera a movimentação como 3 operações
            i++;
            j--;
        }
    }
    if (esq < j)  quicksortRec(pokemon, esq, j);
    if (i < dir)  quicksortRec(pokemon, i, dir);
    
    return movimentacoes;
}

//Chamada para recursividade
int quicksort(Pokemon **pokemon, int n) {
    int movimentacoes = quicksortRec(pokemon, 0, n-1);
    return movimentacoes;
}

void gravarArquivo(double inicio, double fim, int mov){
    FILE *arquivo = fopen("matricula_quicksort.txt", "wt");;
    double total = ((fim - inicio) / (double)CLOCKS_PER_SEC); 

    fprintf(arquivo, "Matricula: 860210\t");  
    fprintf(arquivo, "Numero de Movimentações: %d\t", mov);
...
int main(){
    ...
    inicio += clock();  
    int movimentacoes = quicksort(pokemon, n); 
    fim += clock();
