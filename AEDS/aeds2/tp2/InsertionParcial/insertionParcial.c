/**
 * @path TP02Q16 - Ordenação Parcial por Inserção
...
int compararData(Pokemon *i, Pokemon *j){
    int resp = 0; 
    Date captureDate1 = getCaptureDate(i);
    Date captureDate2 = getCaptureDate(j);

    // Comparar anos
    if (captureDate1.ano < captureDate2.ano) {
        resp = 1; // data1 anterior a data2
    } else if (captureDate1.ano > captureDate2.ano) {
        resp = -1; // data1 posterior a data2
    } else {
        // Se anos iguais, compara os meses
        if (captureDate1.mes < captureDate2.mes) {
            resp = 1; // data1 anterior a data2
        } else if (captureDate1.mes > captureDate2.mes) {
            resp = -1; // data1 posterior a data2
        } else {
            // Se meses iguas, compara os dias
            if (captureDate1.dia < captureDate2.dia) {
                resp = 1; // data1 anterior a data2
            } else if (captureDate1.dia > captureDate2.dia) {
                resp = -1; // data1 posterior a data2
            }
        }
    }
...
//---------------- Metodo para ordenar os k primeiros pokemons pela data - Insertionsort Parcial
void insertionSortParcial(Pokemon **pokemon, int n, int k){
    
    for (int i = 1; i < n; i++) {
        Pokemon *tmp = pokemon[i];
        int j = (i < k) ? i - 1 : k - 1;
        
        while ( (j >= 0) && compararData(tmp, pokemon[j]) == 1){
...
int main(){
    ...
    insertionSortParcial(pokemon, n, 10); 
    print(pokemon, 10);
}
