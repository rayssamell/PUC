...
int movimentacoes = 0; //comparador de movimentacoes
...
void quicksortRec(Pokemon **pokemon, int esq, int dir) {
    int cont = 0;
    int i = esq, j = dir;
    char *pivoNome = getName(pokemon[(dir+esq)/2]); //pivo para comparar nome
    int pivo = getGeneration(pokemon[(dir+esq)/2]); //pivo para comparar generation
 
    while (i <= j) {
        // Primeiro faz a comparacao pela geracao, se for igual compara o nome
        while (getGeneration(pokemon[i]) < pivo || (getGeneration(pokemon[i]) == pivo && strcmp(getName(pokemon[i]), pivoNome) < 0)){ 
            i++;
            comparacoes++;
        }
        while (getGeneration(pokemon[j]) > pivo || (getGeneration(pokemon[j]) == pivo && strcmp(getName(pokemon[j]), pivoNome) > 0)){
            j--;
            comparacoes++;
        }
...
void quicksort(Pokemon **pokemon, int n) {
    quicksortRec(pokemon, 0, n-1);
}
