/**
 * @path TP02Q06 - Selection Sort Recursivo
...
//-------- Metodo para buscar o nome antecessor
int buscarMenor(Pokemon **pokemon, int i, int j) {
    int menor = i;  // Inicializamos menor com o valor de i
  
    if (i != j){
        menor = buscarMenor(pokemon, i + 1, j);
        comparacoes++;
    }
    
    return (strcmp(getName(pokemon[menor]), getName(pokemon[i])) > 0) ? i : menor;

}

//---------------- Metodo para ordenar pokemons pelo nome - Selection Sort Recursivo
void selectionSortRec(Pokemon **pokemon, int n, int i){
    //Caso Base
    if (i < n) {
        // Chamada recursiva para buscar o menor
         
        int menor = buscarMenor(pokemon, i, n - 1);

        // Contando movimentações
        swap(pokemon, i, menor); // Swap para trocar de posicao
        movimentacoes += 3; 

        // Chamada recursiva para o proximo elemento
        selectionSortRec(pokemon, n, i + 1);
    }
   
}

void gravarArquivo(double inicio, double fim){
    FILE *arquivo = fopen("matricula_selecaoRecursiva.txt", "wt");;
...
int main(){
    ...
    // Marcando tempo de execucao para selection sort recursivo //
    inicio += clock();  
    selectionSortRec(pokemon, n, 0); 
    fim += clock();
