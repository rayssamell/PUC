/**
 * @path TP03Q06 - Pilha com alocação Flexivel em c
...
void empilhar(Pilha *pilha, Pokemon *pokemon){
    ...
    tmp->prox = pilha->topo;
    pilha->topo = tmp;
    pilha->n++; //aumentar tamanho
    tmp = NULL;
}

//Reempilhar para inverter ordem da pilha original
void reempilhar(Pilha *pilha, Pilha *pilha2){
    for (Celula *i = pilha->topo; i != NULL; i = i->prox) {
        Celula *nova = (Celula *)malloc(sizeof(Celula));
        nova->elemento = i->elemento;
        nova->prox = pilha2->topo;
        pilha2->topo = nova;
        pilha2->n++;
    }
}

//------------- Remover pokemon da pilha - Desempilhar
Pokemon* desempilhar(Pilha *pilha){
    if(pilha->topo == NULL || pilha->n == 0){
        printf("ERRO: Pilha Vazia\n");
    }
    Pokemon *pk = pilha->topo->elemento;
    Celula *tmp = pilha->topo;
    pilha->topo = pilha->topo->prox;
    pilha->n--; //diminuir tamanho
    tmp->prox = NULL;
    free(tmp);
    tmp = NULL;
    return pk;
...
int main(){
    ...
    //reempilha na nova pilha
    reempilhar(pilha, pilha2);
    print(pilha2); //mostrar pilha
}